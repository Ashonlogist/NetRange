...
## GPS jitter and "stay points"

Consumer GPS is noisy, especially indoors -- a phone sitting still in
one room can report a different lat/lon on every scan, easily 10-30m
of drift. Fed directly into the triangulator, every one of those
jittery fixes becomes its own vertex, so five minutes of standing
still can fragment into a dozen-plus spurious sliver triangles for
what is physically one location -- exactly the "huge mesh out of one
small room" behavior first reported testing this.

Before triangulation, `compute_node_anchors()` walks each device's
scan trail in time order and collapses it into a small number of
stable "anchor" points:

- A new scan merges into the device's current anchor (weighted-average
  position and signal, favoring tighter GPS accuracy and more recent
  scans) if it falls within the anchor's uncertainty radius --
  `max(12m floor, the scan's own reported accuracy)`.
- Once a scan falls outside that radius, the device has genuinely
  moved: the current anchor is finalized and a new one starts.

This is the standard "stay point" pattern from mobility-trace
analysis, adapted here to blend signal readings rather than just
detect dwell locations. The practical effect: a stationary phone
contributes one stable mesh vertex no matter how many times it
rescans, while a phone actually walking around still produces a
distinct vertex at each real stop -- which is what actually lets the
mesh grow the way it's meant to as more real locations get covered.

Note this doesn't (and can't) perfectly separate "real movement" from
"noise" every time -- if a single scan reports high claimed accuracy
(say 8m) but is genuinely 15m off from the device's other readings,
the algorithm has no way to know that accuracy claim was wrong, and
will treat it as movement. That's an inherent limit of single-fix GPS,
...
(`compute_node_anchors`) before triangulation. Because Delaunay
...
- Recency and accuracy weighting shape which scans dominate a device's
  anchor position/signal (`compute_node_anchors`) and the IDW fallback
  path, but the interior interpolation itself (`LinearNDInterpolator`)
  is unweighted linear blending across a triangle's three anchor
  corners. If you want stale/inaccurate anchors to visibly fade
  *within* a triangle too, that's a further refinement, not done yet.
